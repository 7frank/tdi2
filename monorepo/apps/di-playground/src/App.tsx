...
function App() {
  ...
  const extractServices = (): Array<{interfaceName: string; className: string}> => {
    const services: Array<{interfaceName: string; className: string}> = [];

    for (const file of selectedExample.files) {
      const content = editedFiles[file.path] ?? file.content;

      // Look for @Service() decorator followed by class that implements an interface
      const serviceMatch = content.match(/@Service\(\)\s+export\s+class\s+(\w+)\s+implements\s+(\w+)/);
      if (serviceMatch) {
        const [, className, interfaceName] = serviceMatch;
        services.push({ interfaceName, className });
      }
    }
  ...
  // Generate DI_CONFIG.ts content
  const generateDIConfig = (): string => {
    const services = extractServices();

    if (services.length === 0) {
      return `// No services found - add @Service() decorated classes
...
`;
    ...
    const imports = services.map(s =>
      `import { ${s.className} } from './services/${s.className}';`
    ).join('\n');

    const serviceEntries = services.map(s =>
      `    { token: '${s.interfaceName}', implementation: ${s.className} },`
    ).join('\n');

    return `// Auto-generated DI configuration
// This file is generated by the TDI2 playground based on discovered services
