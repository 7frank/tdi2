...
export class TodoService2 implements TodoServiceInterface {
  ...
  async updateTodo(request: UpdateTodoRequest): Promise<void> {
    ...
    try {
      ...
      const action = request.completed !== undefined ? 
        (request.completed ? 'completed' : 'uncompleted') : 'updated';
      this.notificationService.showSuccess(`Todo ${action}: "${updatedTodo.title}"`);
    } catch (error) {
  ...
  async clearCompleted(): Promise<void> {
    ...
    try {
      ...
      this.notificationService.showSuccess(`Cleared ${completedTodos.length} completed todos`);
    } catch (error) {
