...
export class ConfigManager {
  ...
  constructor(options: DIConfigOptions) {
    ...
    // FIXED: Use basename of outputDir for bridge directories instead of hardcoding '.tdi2'
    const bridgeDirName = path.basename(options.outputDir || 'node_modules/.tdi2');
    this.bridgeDirs = options.scanDirs.map(dir => path.resolve(dir, bridgeDirName));
  ...
  private generateDIConfigBridge(bridgeDir: string): void {
    ...
    // FIXED: Ensure relative paths start with ./ for proper ES module resolution
