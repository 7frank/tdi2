...
// Generated: 2025-07-16T16:51:26.380Z
...
export function InlineValueProps(props: {
  services: {
    ...
    const services = props.services ?? (useService('api_Inject_ApiInterface') as unknown as {
        api: Inject<ApiInterface>;
      });
...
export function InlineValueFoo(foo: {
  services: {
    ...
    const services = foo.services ?? (useService('api_Inject_ApiInterface') as unknown as {
        api: Inject<ApiInterface>;
      });
    const api = foo.services?.api ?? (useService('ApiInterface') as unknown as ApiInterface);
  const { services } = foo;
  return <div>{services.api.getData()}Mixed dependencies component</div>;
}
