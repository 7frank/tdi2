...
export class TransformationPipeline {
  ...
  transformComponent(
    ...
    // Step 1.5: NEW - Normalize complex destructuring patterns to simple property access
    // Skip DI-related paths so they can be handled by the DI pipeline
    this.normalizeComplexDestructuring(func, enhancedDependencies);
  ...
  /**
   * NEW: Normalize complex destructuring patterns to simple property access
   * Skips DI-related paths to let the DI pipeline handle them
   */
  private normalizeComplexDestructuring(func: FunctionDeclaration | ArrowFunction, dependencies: ExtractedDependency[] = []): void {
    const body = this.getFunctionBody(func);
    if (!body || !Node.isBlock(body)) return;

    // Build set of DI property paths to skip during normalization
    const diPropertyPaths = new Set<string>();
    for (const dep of dependencies) {
      if (dep.propertyPath && dep.propertyPath.length > 0) {
        diPropertyPaths.add(dep.propertyPath.join("."));
    ...
    const statements = body.getStatements();
    const toReplace: { statement: any; normalizedStatements: string[] }[] = [];
    ...
    for (const statement of statements) {
      if (Node.isVariableStatement(statement)) {
        ...
        for (const declaration of declarations) {
          ...
          // Target: complex destructuring from props
          if (
            Node.isObjectBindingPattern(nameNode) &&
            initializer &&
            Node.isIdentifier(initializer) &&
            ...
            // Check if this destructuring contains any rest parameters
            if (this.hasRestParameters(nameNode)) {
              const normalizedStatements = this.generateNormalizedStatements(nameNode, diPropertyPaths);
              
              if (normalizedStatements.length > 0) {
                toReplace.push({ statement, normalizedStatements });
                
                if (this.options.verbose) {
                  console.log(`🔄 Normalizing destructuring with rest parameters: ${statement.getText()}`);
                  console.log(`   → Generated ${normalizedStatements.length} normalized statements`);
                }
              }
            }
          }
    ...
    // Replace complex destructuring with normalized statements
    for (const { statement, normalizedStatements } of toReplace) {
      const index = statement.getChildIndex();
      
      // Remove original complex statement
      statement.remove();
      
      // Insert normalized statements
      for (let i = normalizedStatements.length - 1; i >= 0; i--) {
        body.insertStatements(index, normalizedStatements[i]);
      }
    }

    if (this.options.verbose && toReplace.length > 0) {
      console.log(`✅ Normalized ${toReplace.length} destructuring patterns with rest parameters`);
    }
  }

  /**
   * Check if destructuring pattern contains any rest parameters
   * If so, it needs normalization regardless of other complexity
   */
  private hasRestParameters(bindingPattern: any): boolean {
    const checkPattern = (pattern: any): boolean => {
      const elements = pattern.getElements();
      ...
      for (const element of elements) {
        if (Node.isBindingElement(element)) {
          ...
          // Found a rest parameter
          if (dotDotDotToken) {
            return true;
          }

          // Check nested patterns recursively
          if (Node.isObjectBindingPattern(nameNode)) {
            if (checkPattern(nameNode)) {
              return true;
            }
          }
        }
    ...
    return checkPattern(bindingPattern);
  }

  /**
   * Generate normalized statements for destructuring with rest parameters
   * Converts to simple property access and rest parameter exclusion functions
   */
  private generateNormalizedStatements(bindingPattern: any, diPropertyPaths: Set<string> = new Set()): string[] {
    const statements: string[] = [];
    const restParams: string[] = [];

    const processPattern = (pattern: any, basePath: string = "props"): void => {
      const elements = pattern.getElements();
      ...
      for (const element of elements) {
        if (Node.isBindingElement(element)) {
          ...
          if (dotDotDotToken) {
            // Rest parameter - handle at the end
            const restName = Node.isIdentifier(nameNode) ? nameNode.getText() : nameNode.getText();
            restParams.push(restName);
            continue;
          }
          ...
          if (propertyNameNode && Node.isIdentifier(propertyNameNode)) {
            // Aliasing: { propertyName: localName }
            propertyName = propertyNameNode.getText();
            ...
            if (propertyNameNode && Node.isIdentifier(propertyNameNode)) {
              ...
              // Skip if this is a DI-related path (like "services")
              if (this.isPathDIRelated(propertyName, propertyName, diPropertyPaths)) {
                if (this.options.verbose) {
                  console.log(`   ⏭️  Skipping DI-related nested destructuring: ${propertyName}`);
                }
                continue;
              }
              
              const nestedPath = `${basePath}?.${propertyName}`;
              processPattern(nameNode, nestedPath);
              extractedProps.push(propertyName);
              continue;
            }
          } else {
            continue; // Skip complex patterns we can't handle
          }

          // Skip if we couldn't extract property info or it's DI-related
          if (!propertyName || !localName) {
            continue;
          }
          ...
          if (this.isPathDIRelated(propertyName, propertyName, diPropertyPaths)) {
            if (this.options.verbose) {
              console.log(`   ⏭️  Skipping DI-related property: ${propertyName}`);
            }
            continue;
          }
          ...
          const accessPath = `${basePath}?.${propertyName}`;
          if (initializer) {
            // With default value
            const defaultValue = initializer.getText();
            statements.push(`const ${localName} = ${accessPath} ?? ${defaultValue};`);
          } else {
            // Simple assignment
            statements.push(`const ${localName} = ${accessPath};`);
          }
        }
      }

      // Generate rest parameter statements
      for (const restName of restParams) {
        const excludeProps = extractedProps.map(prop => `"${prop}"`).join(", ");
        if (excludeProps) {
          statements.push(`const ${restName} = (({ ${extractedProps.join(", ")}, ...rest }) => rest)(${basePath} ?? {});`);
        } else {
          statements.push(`const ${restName} = ${basePath};`);
        }
      }
    };

    processPattern(bindingPattern);
    return statements;
  }
