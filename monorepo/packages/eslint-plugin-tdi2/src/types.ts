/**
 * Type definitions for TDI2 ESLint Plugin
 */
...
export interface MetadataError {
  error: 'CONFIG_NOT_FOUND' | 'PARSE_ERROR';
  message: string;
}

export type MetadataResult = ESLintMetadata | MetadataError;

// Rule options types
export interface InterfaceResolutionOptions {
  showDependencies?: boolean;
  showScope?: boolean;
  showFilePath?: boolean;
  showOtherImplementations?: boolean;
  warnOnAmbiguous?: boolean;
}

export interface ImplementationContextOptions {
  showUsageStats?: boolean;
  showDependencies?: boolean;
  showOtherImplementations?: boolean;
}

export interface InterfaceImplementationsOptions {
  showUsageStats?: boolean;
  showProfiles?: boolean;
  warnOnAmbiguity?: boolean;
}
