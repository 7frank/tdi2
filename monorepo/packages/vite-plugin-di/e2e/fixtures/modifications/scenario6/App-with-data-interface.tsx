...
import type { CounterServiceInterface, DataServiceInterface } from './types/interfaces';
...
export function App(props: AppProps) {
  const {
    services: { counterService, dataService },
  } = props;

  const { count, message } = counterService.state;
  const { data, isLoading } = dataService.state;
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'Arial, sans-serif' }}>
      ...
      <div style={{ marginTop: '20px', padding: '20px', border: '1px solid #ccc' }}>
        <h2>Data Service</h2>
        <p data-testid="data-content">{data}</p>
